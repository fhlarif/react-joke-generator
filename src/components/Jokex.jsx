...
const Jokex = () => {
  ...
  return (
    <div className=" bg-sky-900 shadow-lg shadow-sky-600 lg:w-[56rem] text-slate-200 px-12 py-24 sm:px-24 sm:py-14 rounded-3xl grid gap-3 m-4">
      <h2 className="text-2xl font-bold text-center my-4">Random CN Jokes Generator</h2>
      ...
      {isLoading ? (
        ...
        <>
          <span className="font-semibold">Joke ID: {joke.value.id}</span>
          <p className="font-light tracking-tight leading-loose text-justify">{joke.value.joke}</p>
          <button
            onClick={getJoke}
            type="button"
